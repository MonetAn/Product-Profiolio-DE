...
export interface QuarterData {
  ...
  metricFact: string;
  comment: string;
  /** Доля усилий команды в квартале (0–100), из админки; для таймлайна/treemap при стоимости 0 в выгрузке */
...
export interface RawDataRow {
  ...
  /** Разбивка бюджета по бюджетным подразделениям (table initiative_budget_department_2026) */
...
export interface TreeNode {
  ...
  adminInitiativeRowId?: string;
  /** Ссылка на документацию (для тултипа quick flow / обогащения листьев) */
  documentationLink?: string;
...
export function convertFromDB(
  dbRows: AdminDataRow[],
  budgetAllocationsByInitiativeId?: Record<string, BudgetDepartmentAllocation[]>
): {
  rawData: RawDataRow[];
...
export function periodEffortSum(row: RawDataRow, selectedQuarters: string[]): number {
  let total = 0;
  for (const q of selectedQuarters) {
    total += row.quarterlyData[q]?.effortCoefficient ?? 0;
  }
  return total;
}

/**
 * Вес листа treemap: рубли за период; если 0 — относительный вес по сумме % усилий (новые инициативы до появления cost в выгрузке).
 */
...
export function hasPreliminaryQuarterInPeriod(row: RawDataRow, selectedQuarters: string[]): boolean {
  const qd = row.quarterlyData || {};
  for (const q of selectedQuarters) {
    if (!isQuarterPeriodKey(q)) continue;
    if (qd[q]?.costFinanceConfirmed === false) return true;
  }
  return false;
}

/** Текст для тултипа treemap, карточки инициативы и панели сравнения в Quick Flow (предварительная стоимость). */
export const PRELIMINARY_COST_USER_MESSAGE =
  'Предварительный расчёт стоимости. Данные могут измениться';
...
// Вкладка «Кластеры»: один выбранный кластер → одно дерево (узел кластера под корнем, дальше unit/team/initiative).

function buildClusterStakeholdersUnitsOnlyTree(
  rawData: RawDataRow[],
  ...
  rawData.forEach(row => {
    ...
    if (!unitMap[row.unit]) {
      unitMap[row.unit] = { name: row.unit, value: 0, isUnit: true };
    }
  ...
  return { name: cluster, isStakeholder: true, children };
}

function buildClusterStakeholdersUnitsTeamsTree(
  rawData: RawDataRow[],
  options: BuildTreeOptions,
  cluster: string,
): TreeNode {
...
function buildClusterStakeholdersFullTree(
  rawData: RawDataRow[],
  options: BuildTreeOptions,
  cluster: string,
): TreeNode {
...
function buildClusterStakeholdersUnitsInitiativesTree(
  rawData: RawDataRow[],
  options: BuildTreeOptions,
  cluster: string,
): TreeNode {
...
export function buildStakeholdersTree(rawData: RawDataRow[], options: BuildTreeOptions): TreeNode {
  const emptyRoot: TreeNode = { name: 'Все стейкхолдеры', children: [], isRoot: true };
  if (options.selectedStakeholders.length !== 1) return emptyRoot;

  const cluster = options.selectedStakeholders[0];
  if (!cluster?.trim()) return emptyRoot;
  ...
  let inner: TreeNode;
  if (!showTeams && !showInitiatives) {
    inner = buildClusterStakeholdersUnitsOnlyTree(rawData, options, cluster);
  } else if (showTeams && !showInitiatives) {
    inner = buildClusterStakeholdersUnitsTeamsTree(rawData, options, cluster);
  } else if (showTeams && showInitiatives) {
    inner = buildClusterStakeholdersFullTree(rawData, options, cluster);
  } else {
    inner = buildClusterStakeholdersUnitsInitiativesTree(rawData, options, cluster);
  }
