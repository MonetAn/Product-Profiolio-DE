-- Фикс RLS: старая политика FOR ALL блокировала INSERT пустой cross_initiatives
-- (WITH CHECK требовал user_can_see_cross_initiative до появления members).
-- Плюс RPC create_cross_initiative_with_members — надёжное создание из UI.

-- Снять все известные политики на cross_initiatives (идемпотентно)
...
-- Создание кросс-инициативы + участников одной транзакцией (обходит RLS-ловушки на INSERT/RETURNING).
...
COMMENT ON FUNCTION public.create_cross_initiative_with_members IS
  'Ранний доступ: создать кросс-инициативу и добавить инициативы; доли пересчитываются поровну.';
